...
function check_ram(){
	ret_ram=0
	mem_info=$(ssh ${user}@${machine} "cat /proc/meminfo | egrep 'MemTotal|MemAvailable'")
	total_mem=$(echo ${mem_info} | awk '{print $2}')
	free_mem=$(echo ${mem_info} | awk '{print $5}')
	free_perc=$((${free_mem}/${total_mem}*100))
...
function check_cpu(){
	ret_cpu=0
	cpu_info=$(ssh ${user}@${machine} "top -n 1 | head -3 | tail -1")
	free_cpu=$(echo ${cpu_info} | awk '{print $8}')
...
function check_fs(){
	ret_fs=0
	fs_info=$(ssh ${user}@${machine} "df -Ph | awk '0 + $5 >= ${busy_fs} { print }'")
	if [[ ! -z $(fs_info) ]]
	then
		ret_fs=1
	fi
...
function send_alert(){
	...
	then
		...
		cat dist_list.txt | while read id
		do
...
#main
###################################################################
cat hostname.txt| while read file;
do
	check_variables
	if [[ ${check_var} -eq 0 ]]
	then
		...
		then
			...
			echo "Machine ${hostname} doesn't respond to ping" >${basedir}/log.txt
		fi
		report
		send_alert
	fi
